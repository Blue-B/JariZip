...
export function normalizeJumpit(value, checkedAt) {
  ...
  return {
    ...
    publishedAt: koreaDate(raw.publishedAt), deadline, status: checkedStatus(status, deadline),
...
export function normalizeZighang(value, checkedAt) {
  ...
  return {
    ...
    salary: '미기재', skills: [], publishedAt: koreaDate(raw.createdAt), deadline,
...
export function createJobService({ fetcher = fetch, env = process.env, now = () => new Date() } = {}) {
  ...
  async function search({ provider = 'wanted', query = '', page = 0, location = 'all', category = 'all', experience = 'all', refresh = false } = {}) {
    if (provider !== 'all') requireSource(provider);
    if (typeof query !== 'string' || query.length > 120 || !Number.isInteger(page) || page < 0 || page > 49 || !isJobLocation(location) || !isJobCategory(category) || !isJobExperience(experience)) throw new SourceError('검색 조건을 확인해주세요.', 400, 'BAD_QUERY');
    ...
    if (provider === 'all') {
      ...
      return {
        provider, jobs, nextPage: successful.some(result => result.nextPage !== null) && page < 49 ? page + 1 : null,
    ...
    return obtain(key, async () => {
      ...
      if (provider === 'wanted') {
        ...
        nextPage = (page + 1) * 16 < Number(object(raw.result).totalCount) && page < 49 ? page + 1 : null;
        ...
        nextPage = object(raw.data).last === false && page < 49 ? page + 1 : null;
      ...
      return { provider, jobs, nextPage, checkedAt, warnings, sourceResults: [{ id: provider, name: sources().find(source => source.id === provider).name, count: jobs.length, status: 'ok' }] };
    }, refresh);
  }
