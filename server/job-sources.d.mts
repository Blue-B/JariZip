...
export interface SourceList { provider: SourceProvider | 'all'; jobs: Job[]; nextPage: number | null; checkedAt: string; warnings: string[]; cached: boolean; total?: number; sourceResults: SourceResult[] }
...
export function createJobService(options?: { fetcher?: typeof fetch; env?: Record<string, string | undefined>; now?: () => Date }): {
  ...
  search(options?: { provider?: SourceProvider | 'all'; query?: string; page?: number; location?: string; category?: string; experience?: string; refresh?: boolean }): Promise<SourceList>;
