...
import { fetchSources, searchRemoteJobs, refreshRemoteJob, upsertRemoteJob, type JobSource, type SearchResult, type SourceOption } from '../../lib/remoteJobs';
...
export default function DiscoverJobs() {
  ...
  const [source, setSource] = useState<JobSource>('wanted');
  const [location, setLocation] = useState('all');
  const [sources, setSources] = useState<SourceOption[]>([]);
  const [applied, setApplied] = useState({ query: '', source: 'wanted' as JobSource, location: 'all', page: 0, revision: 0 });
  ...
  useEffect(() => {
    ...
    void searchRemoteJobs(applied.source, applied.query, applied.location, applied.page, controller.signal, applied.revision > 0)
      .then(setResult).catch(problem => { if (!controller.signal.aborted) setError(problem instanceof Error ? problem.message : '공고를 가져오지 못했어요.'); })
  ...
  function submit(event: FormEvent) {
    ...
    setApplied({ query: query.trim(), source, location, page: 0, revision: 0 });
  }
  ...
  const sourceName = applied.source === 'wanted' ? '원티드' : '사람인';
  return <div className="discover-page page-enter">
    <header className="discover-heading"><div><h1>채용 공고</h1><p>실제 공고를 찾고, 준비할 자리를 내 보관함에 모으세요.</p></div><Link className="button secondary" to="/app/jobs"><Bookmark size={15}/>보관한 공고 {state.jobs.filter(job => !job.isDemo).length}</Link></header>
    <form className="discover-search" onSubmit={submit}>
      <label className="discover-query"><Search size={19}/><span className="sr-only">실제 공고 검색어</span><input value={query} maxLength={120} onChange={event => setQuery(event.target.value)} placeholder="직무, 기술 또는 회사 이름"/></label>
      <label><span className="sr-only">공고 출처</span><select value={source} onChange={event => setSource(event.target.value as JobSource)}><option value="wanted">원티드</option><option value="saramin" disabled={!sources.find(item => item.id === 'saramin')?.enabled}>사람인{sources.find(item => item.id === 'saramin')?.enabled ? '' : ' · API 키 필요'}</option></select></label>
      <label><span className="sr-only">공고 근무 지역</span><select value={location} onChange={event => setLocation(event.target.value)}><option value="all">전체 지역</option><option value="seoul">서울</option><option value="gyeonggi">경기</option></select></label>
      <Button type="submit" variant="primary" disabled={loading}>공고 찾기<ArrowRight size={16}/></Button>
    </form>
    <p className="discover-privacy">검색어와 지역만 채용 서비스로 전송해요. 이력서·메모·녹음은 전송하지 않아요.</p>
    <div className="discover-results-heading"><span>{loading ? '출처에서 불러오는 중' : result ? `${sourceName} · ${result.jobs.length}건 · ${applied.page + 1}페이지` : '조회하지 못했어요'}</span><Button variant="ghost" disabled={loading} onClick={() => setApplied(value => ({ ...value, revision: value.revision + 1 }))}><RefreshCw size={14}/>다시 조회</Button></div>
    ...
    {result && <>
      <div className="discover-source-time"><Clock3 size={14}/><span>출처 조회 {timeLabel(result.checkedAt)}{result.cached ? ' · 1분 이내 캐시' : ''}</span><span>접수 중으로 응답한 공고만 표시</span></div>
      <div className="discover-list">{result.jobs.map(job => <button type="button" className="discover-job" key={job.id} onClick={() => void openDetail(job)}>
        <CompanyMark job={job}/><span className="discover-job-main"><span className="discover-company">{job.company}</span><strong>{job.title}</strong><span className="discover-meta"><span><MapPin size={13}/>{job.location}</span><span>{job.experience}</span>{job.employment !== '미기재' && <span>{job.employment}</span>}</span></span>
      ...
      {!result.jobs.length && <EmptyState title="조건에 맞는 접수 중 공고가 없어요" description="검색어를 줄이거나 다른 지역을 선택해보세요. 마감·미확인 결과는 표시하지 않아요." action={<Button onClick={() => { setQuery(''); setLocation('all'); setApplied({ query: '', source, location: 'all', page: 0, revision: 0 }); }}>최신 공고 보기</Button>}/>}
      ...
      <div className="discover-footnotes">{result.warnings.map(warning => <p key={warning}>{warning}</p>)}<p>접수 상태는 조회 시점의 출처 응답이며, 실제 지원 직전에는 원본을 확인해주세요.</p></div>
    </>}
    <details className="discover-connection"><summary>연결 방식과 조회 제한</summary><p>원티드의 로그인 없이 공개되는 공고 응답을 조회합니다. 공식 제휴 API가 아니므로 형식 변경이나 조회 제한이 생길 수 있어요. 차단이나 인증 요구는 우회하지 않습니다.</p><p>사람인은 공식 API 키를 서버의 <code>SARAMIN_ACCESS_KEY</code>에 설정하면 사용할 수 있어요. 키를 브라우저 저장소나 GitHub에 넣지 마세요.</p></details>
    {detail && <Modal title={detail.title} description={`${detail.company} · ${detail.source}`} size="drawer" onClose={closeDetail}>
      ...
      {detailLoading ? <p className="discover-loading" role="status">출처에서 공고 본문을 가져오는 중이에요.</p> : <>
        ...
        <dl className="discover-facts"><div><dt>근무지</dt><dd>{detail.location}</dd></div><div><dt>경력</dt><dd>{detail.experience}</dd></div><div><dt>게시일</dt><dd>{detail.publishedAt ? formatDate(detail.publishedAt) : '출처 미제공'}</dd></div><div><dt>마감일</dt><dd>{detail.deadline ? formatDate(detail.deadline) : '날짜 미제공 · 원본 확인'}</dd></div></dl>
