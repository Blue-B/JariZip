...
import { refreshRemoteJob, upsertRemoteJob, canRefreshJob } from '../../lib/remoteJobs';
...
export default function Jobs() {
  ...
  const recheck = async (job: Job) => {
    if (!canRefreshJob(job)) { notify('이 출처는 제공사 사전 승인이 없어 자동으로 조회하지 않아요. 원문 사이트에서 직접 확인하고 접수 상태를 기록해주세요.'); return; }
    setRefreshing(true);
    try {
