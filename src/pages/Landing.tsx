...
const SPACES = [
  { to: '/app/discover', label: '채용 탐색', body: '여러 채용 사이트의 공고를 지역·직무별로 찾고 관심 공고로 보관합니다.' },
...
const FLOW = [
  { n: '01', title: '공고 찾기', body: '원티드·점핏·직행의 실제 공고를 함께 찾습니다. 전국 지역, 직무 분야와 경력 조건으로 좁혀 보세요.', to: '/app/discover', cta: '채용 탐색 열기' },
...
const OUTBOUND = ['검색어와 지역·직무·경력 필터', '선택한 출처의 공개 공고 조회 요청', '사람인 키는 서버에서 해당 출처에만 전송'];
const LOCAL = ['이력서·자기소개서 원문과 업로드한 파일', '면접 답변과 녹음', '지원 기록, 기업 메모, 설정', 'JSON 백업 파일'];

const LIMITS = [
  { icon: KeyRound, title: '사람인은 키가 있을 때만 연결합니다', body: '원티드·점핏·직행은 별도 키 없이 공개 공고를 조회합니다. 사람인은 서버에 직접 발급받은 API 키를 설정한 경우에만 추가됩니다.' },
...
const FAQ = [
  { q: '공고는 어디에서 가져오나요?', a: '원티드·점핏·직행의 공개 목록과 상세 응답을 읽어 옵니다. 사람인은 서버에 API 키를 설정한 경우에 추가됩니다. 검색어와 필터만 외부로 전송하고, 이력서와 녹음은 전송하지 않습니다. 출처마다 검색 범위가 다르고 일부 출처가 실패하면 결과와 함께 알려드립니다.' },
    ...
    <section className="lz-hero" aria-labelledby="lz-hero-title">
      <div className="lz-shell lz-hero-grid">
        <div className="lz-hero-copy">
          <p className="lz-hero-kicker">여러 채용 사이트를 한곳에서 찾는 취업 워크스페이스</p>
